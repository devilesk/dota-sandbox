...
function CHeroDemo:DumpModifiers()
  ...
  if hPlayerHero == nil then return 1 end
  DebugPrint("FindAllModifiers")
  for k, v in pairs( hPlayerHero:FindAllModifiers() ) do
    DebugPrint (k)
    --DebugPrint(hPlayerHero:GetModifierNameByIndex(k - 1))
    DebugPrint(v:GetName())
    --DebugPrint(v:GetClass())
    --DebugPrint(v:GetRemainingTime())
    DebugPrint(v:GetStackCount())
    --DebugPrint(v:GetCaster():GetName())
    if v:GetAbility() then DebugPrint(v:GetAbility():GetName()) end
    --DebugPrint("GetIntrinsicModifierName" .. v:GetAbility():GetIntrinsicModifierName())
    --DebugPrintTable(v, " ")
  end
  return 1
end
